...
// Side-effect: daftarkan Hello Node ke nodeRegistry
import './definitions';
...
export const HelloPlugin: React.FC = () => {
  ...
  // ── Proof #2: Shortcut Registration (context-sensitive) ────────────────
  // Ctrl+H bekerja saat Hello Plugin editor sedang terbuka.
  // Saat editor ditutup, shortcut ini dilepas (unmount cleanup).
  // ─────────────────────────────────────────────────────────────────────
  useEffect(() => {
    shortcutRegistry.register({
      key: 'ctrl+h',
      commandId: 'hello.greet',
      handler: () => addNotificationRef.current('success', '👋 Hello from Hello Plugin! (Ctrl+H)'),
    });

    return () => {
      shortcutRegistry.unregister('ctrl+h');
    };
  }, []);

  // ── Proof #3: Context Menu Registration ────────────────────────────────
  // Canvas right-click menu grup untuk Hello Plugin.
  // ─────────────────────────────────────────────────────────────────────
  useEffect(() => {
      ...
      items: [
        {
          ...
          label: 'Say Hello',
          icon: 'Sparkles',
          handler: () => addNotificationRef.current('success', '👋 Hello from context menu!'),
        },
        {
          ...
          icon: 'Activity',
          separator: true,
          handler: () => addNotificationRef.current('info', '✅ All 6 platform services verified!'),
        },
      ],
    });

    return () => {
      contextMenuRegistry.unregisterGroup('hello.canvas');
    };
  }, []);
